...
uint16_t getSensorSubtree(std::shared_ptr<SensorSubTree>& subtree)
{
    ...
    auto checksdrUpdateSensorTree = [&dbus]() {
        ...
        constexpr auto availabilityInterface =
        ...
        for (const auto& sensor : *sensorTreePtr)
        {
            ...
            {
                // Default set the sensor available as true if the 'available'
                               ...
                               std::array<uint8_t, eventDataSize>& eventData)
{
    ...
    if (eventType == 0x6f)
    {
        ...
        sizeOfEvents = sizeof(sensor_specific_event_types) /
                       sizeof(sensor_specific_event_types[0]);
    }
    else
    {
        evt = generic_event_types;
        sizeOfEvents = sizeof(generic_event_types) /
                       sizeof(generic_event_types[0]);
    }

    uint8_t offset = eventData[0] & 0xf;
    for (uint32_t i = 0; i < sizeOfEvents; ++i, ++evt)
    {
        if ((evt->code == sensorType) && (evt->offset == offset && evt->desc) &&
