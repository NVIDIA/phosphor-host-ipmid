    ...
    uint8_t* mac, size_t* macLen, unsigned char* outBytes, size_t* outBytesLen)
{
    ...
    // Set key & IV
    int retval = EVP_CipherInit_ex(
        ctx.get(), cipher, nullptr, key, iv, static_cast<int>(doEncrypt));
    if (!retval)
    {
    ...
    if ((retval = EVP_CipherUpdate(
             ctx.get(), outBytes + outLen, &outEVPLen, inBytes, inBytesLen)))
    {
        outLen += outEVPLen;
        if ((retval =
    ...
    if (inBytesLen != 0)
    {
        ...
        while (linePtr != nullptr)
        {
            ...
            if ((userEPos = lineStr.find(":")) != std::string::npos)
            {
                ...
                {
                    bytesWritten += std::snprintf(
                        &inBytes[0] + bytesWritten, (inBytesLen - bytesWritten),
                        "%s\n", lineStr.data());
                }
            }
            linePtr = strtok_r(nullptr, "\n", &nToken);
